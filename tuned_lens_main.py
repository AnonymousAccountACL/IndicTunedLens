...
def save_logits_to_csv(lens, text, lens_name, k=10, prompt_id=None):
    ...
    filename = f"tunedlens/results/{lens_name.lower()}_logits_probabilities{'_' + str(prompt_id) if prompt_id is not None else ''}.csv"
...
def process_csv_prompts(csv_path, k=10, head=5):
    ...
    for idx, row in df_input.iterrows():
        ...
        df_logit = save_logits_to_csv(logit_lens, text, "LogitLens", k=k, prompt_id=prompt_id)
        logit_dfs.append(df_logit)
    
    if tuned_dfs:
        combined_tuned = pd.concat(tuned_dfs, ignore_index=True)
        combined_tuned_filename = "tunedlens/results/tunedlens_combined_logits_probabilities.csv"
    ...
    if logit_dfs:
        ...
        combined_logit_filename = "tunedlens/results/logitlens_combined_logits_probabilities.csv"
        combined_logit.to_csv(combined_logit_filename, index=False, escapechar='\\', quoting=1, encoding='utf-8')

csv_path = "tunedlens/data/sft_train.csv"
process_csv_prompts(csv_path, k=10, head=5)