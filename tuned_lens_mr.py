...
def save_logits_to_csv(lens, text, lens_name, k=10, prompt_id=None):
    ...
    filename = f"tunedlens/results/mr/{lens_name.lower()}_logits_probabilities{'_' + str(prompt_id) if prompt_id is not None else ''}.csv"
...
def process_csv_prompts(csv_path, k=10, head=5):
    ...
    if tuned_dfs:
        ...
        combined_tuned_filename = "tunedlens/results/mr/tunedlens_combined_logits_probabilities.csv"
        combined_tuned.to_csv(combined_tuned_filename, index=False, escapechar='\\', quoting=1, encoding='utf-8')

csv_path = "tunedlens/data/m_mmlu_mr.csv"
